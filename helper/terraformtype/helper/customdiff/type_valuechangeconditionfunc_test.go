...
func TestIsFuncTypeValueChangeConditionFunc(t *testing.T) {
	boolIdent := &ast.Ident{
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			got := IsFuncTypeValueChangeConditionFunc(testCase.Node, testCase.Info)
...
func TestIsTypeValueChangeConditionFunc(t *testing.T) {
	testCases := []struct {
		Name      string
		...
		{
			Name: fmt.Sprintf("%s.%s", PackagePath, TypeNameValueChangeConditionFunc),
			InputType: types.NewNamed(
				types.NewTypeName(token.NoPos, types.NewPackage(PackagePath, PackageName), TypeNameValueChangeConditionFunc, nil),
				nil,
				nil,
			),
			Expected: true,
		...
		{
			Name: fmt.Sprintf("%s.%s", "incorrect/path", TypeNameValueChangeConditionFunc),
			InputType: types.NewNamed(
				types.NewTypeName(token.NoPos, types.NewPackage("incorrect/path", "path"), TypeNameValueChangeConditionFunc, nil),
				nil,
				nil,
			),
			Expected: false,
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			got := IsTypeValueChangeConditionFunc(testCase.InputType)
